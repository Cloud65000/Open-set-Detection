...
def mAP_metric(server_ip):
    ...
    val_data = '/media/oyrq/4T/Data/all_test_data/smoke_fire'  #数据集测试格式和训练集一致
    ...
    if label_merge:
        ...
        names_merge_dic = {k: v for k, v in enumerate(names_merge_list)}
        names_dic = names_merge_dic
        print(names_dic)
    ...
    for idx, path in enumerate(paths):
        ...
        rects, cls, confs = obj_detect(image, prompt , 0.25)
