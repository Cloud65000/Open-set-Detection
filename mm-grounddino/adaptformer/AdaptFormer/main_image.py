...
from timm.models.layers import trunc_normal_#这里引入的东西
...
def main(args):
    ...
    for name, p in model.named_parameters(): #这里的冻结内容可做lora参考
        if name in msg.missing_keys: #加入的apdaptformer结构未冻结
            p.requires_grad = True
        else:
            p.requires_grad = False if not args.fulltune else True
    for _, p in model.head.named_parameters(): #头部未冻结
        p.requires_grad = True

    model.to(device)

    model_without_ddp = model
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad) #获取未冻结参与微调的参数个数
    ...
    optimizer = torch.optim.SGD([p for name, p in model.named_parameters() if p.requires_grad], lr=args.lr, weight_decay=args.weight_decay, momentum=0.9)  #这里是参数优化器的可参考处
    print(optimizer)
