...
from sub_Target import get_Target_sub
import torch
import argparse
import numpy as np
...
def mAP_metric(server_ip):
    ...
    prompt=[ 'person', 'delivery person', 'lying', 'squatting', 'sitting', 
            ...
            'pickup', 'RV', 'caravan', 'truck', 'van', 'tractor', 'muck truck', 
            'fence type vehicle', 'flatbed car', 'stake car', 'rescue trailer', 'transport trailer',
    ...
    for idx, path in enumerate(paths):
        ...
        pre_Target = read_xml(xml_path, names)
        preTarget_len=len(pre_Target)
        Target=[]
        for i in range(preTarget_len):
            wait_filt=pre_Target[i][0]
            check_target=get_Target_sub(wait_filt,names_merge,0,22)
            if check_target:
        ...
        if label_merge:
            ...
            for tar in Target:
                print(tar)
                if tar[0] not in merged_tar:
                    merged_tar.append(tar[0])
            print(merged_tar,len(merged_tar))
                # if Target
